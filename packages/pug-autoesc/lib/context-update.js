...
var {
  ATTR_TYPE_ALL,
  ATTR_TYPE_NONE,
  ATTR_TYPE_PLAIN_TEXT,
  ATTR_TYPE_SCRIPT,
  ATTR_TYPE_STYLE,
  ATTR_TYPE_URI,
  CONTENT_KIND_FOR_ESC_MODE,
  CONTENT_KIND_HTML,
  CONTENT_KIND_JS_STR_CHARS,
  CONTENT_KIND_URI,
  DELIM_TEXT,
  DELIM_TYPE_ALL,
  DELIM_TYPE_DOUBLE_QUOTE,
  DELIM_TYPE_NONE,
  DELIM_TYPE_SINGLE_QUOTE,
  DELIM_TYPE_SPACE_OR_TAG_END,
  ELEMENT_TYPE_ALL,
  ELEMENT_TYPE_LISTING,
  ELEMENT_TYPE_NONE,
  ELEMENT_TYPE_NORMAL,
  ELEMENT_TYPE_SCRIPT,
  ELEMENT_TYPE_SCRIPT_CUSTOM,
  ELEMENT_TYPE_STYLE,
  ...
  ESC_MODE_ESCAPE_JS_STRING,
  ESC_MODE_ESCAPE_JS_VALUE,
  ESC_MODE_ESCAPE_URI,
  ESC_MODE_FILTER_CSS_VALUE,
  ESC_MODE_FILTER_HTML_ATTRIBUTE,
  ESC_MODE_FILTER_HTML_ELEMENT_NAME,
  ESC_MODE_FILTER_NORMALIZE_URI,
  ESC_MODE_FOR_STATE,
  ESC_MODE_NORMALIZE_URI,
  ESC_MODE_NO_AUTOESCAPE,
  IS_ESC_MODE_HTML_EMBEDDABLE,
  JS_FOLLOWING_SLASH_ALL,
  JS_FOLLOWING_SLASH_DIV_OP,
  JS_FOLLOWING_SLASH_NONE,
  JS_FOLLOWING_SLASH_REGEX,
  JS_FOLLOWING_SLASH_UNKNOWN,
  STATE_ALL,
  STATE_CSS,
  STATE_CSS_COMMENT,
  STATE_CSS_DQ_STRING,
  ...
  STATE_HTML_BEFORE_ATTRIBUTE_VALUE,
  STATE_HTML_BEFORE_TAG_NAME,
  STATE_HTML_COMMENT,
  STATE_HTML_NORMAL_ATTR_VALUE,
  STATE_HTML_PCDATA,
  STATE_HTML_RCDATA,
  STATE_HTML_TAG,
  STATE_HTML_TAG_NAME,
  STATE_JS,
  STATE_JS_BLOCK_COMMENT,
  STATE_JS_DQ_STRING,
  STATE_JS_LINE_COMMENT,
  STATE_JS_REGEX,
  STATE_JS_SQ_STRING,
  STATE_URI,
  URI_PART_ALL,
...
function contextToString(context) {
  ...
  case ELEMENT_TYPE_SCRIPT_CUSTOM: parts.push("SCRIPT_CUSTOM"); break;
...
var processRawText = (function () {
  ...
  TAG_DONE_ELEMENT_TYPE_TO_PARTIAL_CONTEXT[ELEMENT_TYPE_SCRIPT_CUSTOM]
      = STATE_HTML_RCDATA;
  ...
  var JS_MIME_TYPE = /^\s*(application\/(?:x-)?(?:ecma|java|live)script|text\/ecmascript|text\/javascript(?:1[.][0-5])?|text\/jscript|text\/x-(?:ecma|java)script)\s*$/i;

  /**
   * A transition that takes into account whole type attributes.
   * @param {RegExp} regex A regular expression whose group 1 is the encoded
   *     attribute value.
   * @constructor
   * @extends Transition
   */
  function TypeAttrTransition(regex) {
    Transition.call(this, regex);
  }
  TransitionSubclass(
      TypeAttrTransition,
      function (prior, match) {
        var el = elementTypeOf(prior);
        if (el === ELEMENT_TYPE_SCRIPT
            || el === ELEMENT_TYPE_SCRIPT_CUSTOM) {
          var attrValue = match[1].toLowerCase();
          var ch0 = attrValue.charAt(0);
          if (ch0 === '"' || ch0 === '\'') {
            attrValue = attrValue.substring(1, attrValue.length - 1);
          }
          attrValue = unescapeHtml(attrValue);
          if (JS_MIME_TYPE.test(attrValue)) {
            el = ELEMENT_TYPE_SCRIPT;
          } else {
            el = ELEMENT_TYPE_SCRIPT_CUSTOM;
            // https://html.spec.whatwg.org/#scriptingLanguages says
            // Servers ... must not use non-JavaScript MIME types.
            // TODO: prevent interpolation in custom script elements
            // since behavior is undefined.
          }
        }
        return STATE_HTML_TAG | el;
      });
  ...
  TRANSITIONS[STATE_HTML_TAG] = [
    new TypeAttrTransition(
      new RegExp('^\\s*type\\s*=\\s*(' + FULL_ATTRIBUTE_VALUE + ')')),
    ...
    // comment end ("-->").
    new TransitionToAttrName(/^\s*([:a-z_@][\w:@\-]*)/i),
...
exports.computeEscapingModeForSubst = computeEscapingModeForSubst;
exports.contextToString = contextToString;
exports.processRawText = processRawText;
